...
\begin{equation}
 Z_{bulk}(\phi_0)=\left\langle\exp(\i\int\d^dx\sqrt{g_0}\phi_0\mathcal{O})\right\rangle_{FT}\label{fulCorr}
\end{equation}
...
The expectation value on the right-hand side is of a field theory at a temperature given by the periodicity of the path integral for the partition function. The field $\phi_0$ is source to the operator $\mathcal{O}$,
\begin{equation}
\mathcal{O}= \frac{\delta S_{CFT}}{\delta \phi_0}
\end{equation}
where $S_{CFT}$ is the CFT action.
\\
The boundary theory becomes classical for a gauge boundary theory with a large number of colors, a large $N$. We do not have a large-$N$ theory but a similar effect is expected for strongly coupled boundary theories so a classical bulk theory is assummed. See for example \cite{McGreevy:2009xe} for a thorough treatment of when the bulk theory can be considered classical. The partition function is then given in a semi-classical limit by
\begin{equation}
 Z_{bulk}(\phi_0)=C\exp(\i S_c)
\end{equation}
where $S_c$ is the bulk theory action for the classical periodic path in Euclidean time\cite{hartnoll8} and $C$ is a constant. This path has boundary condition at the conformal boundary described by $\phi_0$.\\
Expectation values of the CFT operator $\mathcal{O}(x)$ can be calculated by
\begin{equation}
\begin{split}
\frac{\delta S_c(\phi_0)}{\delta\phi_0(x)}|_{\phi_0=0}&=
-\i\frac{\delta\log Z_{bulk}(\phi_0)}{\delta\phi_0(x)}|_{\phi_0=0}\\
&=-\i\frac{\delta\log\left\langle\exp(\i\int\d^dx\sqrt{g_0}\phi_0\mathcal{O})\right\rangle_{FT}}{\delta\phi_0(x)}|_{\phi_0=0}\\
&=\frac{\left\langle\sqrt{g_0}\phi_0\exp(\i\int\d^dx\sqrt{g_0}\phi_0\mathcal{O})\right\rangle_{FT}}{\left\langle\exp(\i\int\d^dx\sqrt{g_0}\phi_0\mathcal{O})\right\rangle_{FT}}|_{\phi_0=0}\\
&=\langle \sqrt{g_0}\mathcal{O}(x) \rangle_{FT}
\end{split}
\end{equation}
where the first equality comes from the semi-classical approximation and the second equality from \eqref{fulCorr}.
...
\end{equation}
where $g$ is the absolute value of the determinant of the metric tensor, $g=|\det g_{ab}|$. $S_\mathrm{boundary}$ is a boundary term that is needed to cancel divergences when integrating the action towards the boundary. It does not affect the equations of motion but is needed to get normalizeable modes.\\
...
\begin{equation}
\begin{split}
\langle \mathcal{O}(\psi(x))\rangle&=-\i\frac{\delta}{\delta J(x)}\log(Z_{\mathrm{CFT}}[J])|_{J=0}\\
&\stackrel{\mathrm{GKPW}}{=}-\i\frac{\delta}{\delta J(x)}\log(Z_{\mathrm{Bulk}}[J])|_{J=0}\\
&\stackrel{\mathrm{classical}}{=}\frac{\delta}{\delta J(x)}S_c|_{J=0}
\end{split}
\end{equation}
where $J$ is the field coupling to the operator $\mathcal{O}$ in the field theory. $S_c$ here is the on-shell action. 
...
\begin{equation}
\begin{split}
\frac{\delta}{\delta J(x)}S_c|_{J=0}=\int \d^{d+1}y\sqrt{g}\left(\frac{\partial\L(y)}{\partial \phi_i(y)}\frac{\partial \phi_i(y)}{\partial J(x)}+\frac{\partial\L(y)}{\partial (\nabla_a\phi_i(y))}\frac{\partial (\nabla_a\phi_i(y))}{\partial J(x)}\right)
\end{split}
\end{equation}
where $i$ goes over all fields. Here the Lagrangian is assumed to only depend on the fields and their first derivatives. Now integrate by parts
\begin{equation}
\begin{split}
\frac{\delta}{\delta J(x)}S_c|_{J=0}=&\int \d^{d+1}y\sqrt{g}\left(\frac{\partial\L(y)}{\partial \phi_i(y)}-\nabla_a\frac{\partial\L(y)}{\partial (\nabla_a\phi_i(y))}\right)\frac{\partial (\phi_i(y))}{\partial J(x)}\\
+&\int_{\partial\mathrm{AdS}} \d^{d}y\sqrt{g_0}n_a\frac{\partial\L(y)}{\partial (\nabla_a\phi_i(y))}\frac{\partial (\phi_i(y))}{\partial J(x)}
\end{split}
\end{equation}
