...
\begin{empheq}[left=\empheqlbrace]{align}
 &\psi=\psi_{(h)}s^{\Upsilon_\psi}\\
 &\phi=\phi_{(h)}s^{\Upsilon_\phi}\\
 &A_x=A_{x(h)}s^{\Upsilon_{A_x}}
\end{empheq}
...
\begin{empheq}[left=\empheqlbrace]{align}
 &q^2z^2\phi_{(h)}^2s^{2\Upsilon_\phi}+9z_h^2\Upsilon_\psi+z_h^2 9\Upsilon_\psi(\Upsilon_\psi-1)=0\\
 &-2q^2\psi_{(h)}^2s^{2\Upsilon_\psi}L^2+z_h^2 3s^{-1}\Upsilon_\phi(\Upsilon_\phi-1)=0\\
 &-6q^2\psi_{(h)}^2s^{2\Upsilon_\psi+1}L^2+z_h^2\omega^2+z_h^2 9\Upsilon_{A_x}+z_h^2 9\Upsilon_{A_x}(\Upsilon_{A_x}-1)=0.
\end{empheq}
...
The horizon parameters can now be varied to find solutions to these two boundary values and there are no free parameters. See Appendix \ref{num} for a description of the numerical integration.\\

There is an analytical trivial solution of the equations of motion and boundary conditions.
\begin{equation}
\begin{split}
 &\psi(z)=0\\
 &\phi(z)=\mu(1-z/z_h)\\
...
\frac{\sqrt{z^2+zz_h+z_h^2}}{z_h-z}
\right]^{\pm\frac{\i\omega z_h}{3} }   \label{trivial}
\end{split}
\end{equation}
...
Expectation values of field theory operators can now be calculated using these solutions and \eqref{expectation}. Not just the leading behaviour of the fields is needed to calculate the expectation values but also the first subleading behaviour. Therefore we expand the fields as
\begin{empheq}[left=\empheqlbrace]{align}
