...
%It might be possible to see the $\delta$-function in the conductivity as a spontanous symmetry breaking just like for the operator $\mathcal{O}$, an expectation value appearing without a source. The symmetry would be invariance The infinite conductivity should appear when $\omega=0$ and $\psi>0$. This is precisely when the terms without derivatives in the differential equation \eqref{eqm3} becomes negative near the boundary.
\begin{appendices}
