...
fig(11)
Trs=list(np.linspace(0.2,1,5))+[1.3]
zh=1
T=3./(zh*4.*np.pi)
wvs= [1e-4,1e-3]+list(np.linspace(1e-3,1,100))[1:]+list(np.logspace(0,1.5,40))[1:]
#pl.xscale('log')
#pl.yscale('log')
first=True
y1s=[]
...
for j in range(len(Trs)):
    ...
    rhoSol=rhoSol[0]
    sigmas=[]
    bb,osc=getBoundary(rhoSol[0],rhoSol[1], [0,a2])
    mu=bb[1][0]
    sigmas=[]
    ...
    pl.plot(wvs,[s.real-1 for s in sigmas],ls='-',c='k',label=r'$\mathrm{Im}(\sigma)$'if first else '',marker='.')
    first=False
