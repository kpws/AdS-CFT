...
psiMin=1e-5
for mu in mus:
    phiDs=np.linspace(-0.01,-0.02,20)
    ys=[]
    for phiD in phiDs:
        print str(psi)+':'
        f=lambda psi:getBoundary(phiD,psi)[2]
        a=f(psiMin)
        end=0.1
        while a*f(end)>=0:
            end=end*1.2
        sol=brentq(f,psiMin,end,xtol=1e-4)
        pl.figure(2)
        y=mu,rho,p1,p2=getBoundary(phiD,sol,plot=True)
        ys.append(y)
    pl.figure(1)
